...
<#
        ...
        .EXAMPLE
        The Output can be formatted with Format-Table. However, it should first be sorted so that the property is properly seperated.
    ...
    Process { 
        foreach ( $PathString in $Path ) 
        {
            ...
            Foreach ($Rule in $AccessRule)
            {
                ...
                $AccountObjects = New-Object -TypeName PSObject -Property $Props
